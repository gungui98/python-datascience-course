...
def calculate_demographic_data(print_data=True):
    ...
    cond = df['salary'] == '>50K'
    cond2 = df['hours-per-week'] == min_work_hours
    rich_percentage = round((cond&cond2).sum()/cond2.sum()*100, 1)

    # What country has the highest percentage of people that earn >50K?
    cond3 = (df[cond]["native-country"].value_counts()/df["native-country"].value_counts()*100).sort_values(ascending=False)
    highest_earning_country = cond3.index[0]
    highest_earning_country_percentage = round(cond3.iloc[0], 1)
