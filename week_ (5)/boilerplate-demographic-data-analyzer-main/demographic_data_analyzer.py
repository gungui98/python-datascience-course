...
def calculate_demographic_data(print_data=True):
    ...
    cond = df['salary'] == '>50K'
    df_rich = (df.loc[cond, 'native-country'].value_counts() / df['native-country'].value_counts()).sort_values(ascending = False)
    highest_earning_country = df_rich.head(1).index[0]
    highest_earning_country_percentage = round(df_rich[highest_earning_country] * 100, 1)

    # Identify the most popular occupation for those who earn >50K in India.
    cond1 = df['native-country'] == 'India'
    cond2 = df['salary'] == '>50K'
    top_IN_occupation = df.loc[(cond1 & cond2), 'occupation'].value_counts().head(1).index[0]
