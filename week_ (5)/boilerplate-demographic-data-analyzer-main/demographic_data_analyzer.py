...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = round(df[(df.education.isin(['Bachelors', 'Masters', 'Doctorate']))].shape[0] / df.shape[0]  * 100, 1)

    
    lower_education = round(df[~(df.education.isin(['Bachelors', 'Masters', 'Doctorate']))].shape[0] / df.shape[0]  * 100, 1)
    ...
    num_min_workers = df[df['hours-per-week'] == min_work_hour].shape[0]

    rich_percentage = df[(df['hours-per-week'] == min_work_hour) & (df['salary'] == '>50K')].shape[0] / num_min_workers *100
    ...
    lmao2 = df[(df['native-country'] == 'India') & (df['salary'] == '>50K') ]['occupation'].value_counts()
    lmao2.sort_values(ascending= False)
    top_IN_occupation = lmao2.index[0]
