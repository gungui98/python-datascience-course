...
def calculate_demographic_data(print_data=True):
    ...
    percentage_each_country = (salary_50K['native-country'].value_counts() / df['native-country'].value_counts() * 100).sort_values(ascending=False)
  
    highest_earning_country = percentage_each_country.index[0]
    highest_earning_country_percentage = round(percentage_each_country[highest_earning_country], 1)

    # Identify the most popular occupation for those who earn >50K in India.
    occupation_in_India = (df[(df["native-country"] == "India") & (df["salary"] == ">50K")])['occupation'].value_counts().sort_values(ascending=False)
